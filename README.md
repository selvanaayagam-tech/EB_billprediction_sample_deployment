...
## 4. Billing Logic
Bills are computed using the LT Commercial energy tariff:

```python
if monthly_units <= 100:
    energy_charge = monthly_units * 5.50
elif monthly_units <= 250:
    energy_charge = (100 * 5.50) + (monthly_units - 100) * 6.50
else:
    energy_charge = (100 * 5.50) + (150 * 6.50) + (monthly_units - 250) * 7.20

price_rs = energy_charge + 120  # fixed monthly charge
```

**Note:** This tariff rate is assumed constant across all 9 years of the dataset for simplicity. In reality, electricity tariffs are revised periodically (typically every 1-3 years) by the state electricity board, so this is a simplifying assumption, not a realistic reflection of actual billing history.

This calculation is applied **after** the model's prediction, not learned by the model itself, since it's a fixed, known formula rather than a pattern to be inferred from data.
